...
DEFAULT_CONFIG: Dict = {
    ...
    "cfbd_api_key": "",                 # free key from collegefootballdata.com (1,000 calls/mo)
                                        # PREFER config.local.json or env CFBD_API_KEY — do not
                                        # commit a real key into this tracked file.
    "regions": "us",
...
def load_config() -> Dict:
    ...
    # a gitignored local file overrides config.json for secrets, so a real key
    # never lives in the tracked config.json (same idea as .env)
    local_path = os.path.join(ROOT, "config.local.json")
    if os.path.exists(local_path):
        try:
            with open(local_path) as f:
                cfg.update(json.load(f))
        except Exception as exc:  # noqa: BLE001
            print(f"[config] could not parse config.local.json ({exc}); ignoring")
    # env vars override files, handy for scheduled/CI runs
    if os.environ.get("ODDS_API_KEY"):
        cfg["odds_api_key"] = os.environ["ODDS_API_KEY"]
    if os.environ.get("CFBD_API_KEY"):
        cfg["cfbd_api_key"] = os.environ["CFBD_API_KEY"]
    return cfg
